...
io.on("connection", (socket) => {
  ...
  socket.on(
    "mapData",
    ({ tileIndices, tileWidth, tileHeight, mapWidth, mapHeight, scale }) => {
  ...
  socket.on("joinRoom", (data) => {
    ...
    socket.to(gameId).emit("playerInGameMap", {
      message: `Player ${player.name} connected to your '${gameId}' game room!`,
    });
    console.log(
      `game_ConsoleLog: Player ${player.name} connected to game room: ${gameId}`
    );
  });
  ...
  socket.on("clientPlayerUpdate", (playerData) => {
    ...
    socket.to(gameId).emit("playerUpdates", {
      id: playerData.id,
      x: playerData.playerX,
      y: playerData.playerY,
      activeKeys: playerData.activeKeys,
  ...
  socket.on("createGameRoom", (gameId) => {
    ...
    socket.join(gameId);
    console.log(
      `lobby_ConsoleLog: Player with socketID: ${socket.id} joined game room ${gameId}`
    );

    // Notify the player that they've joined the room
    socket.emit("gameRoomCreated", {
      gameId: gameId,
      message: `lobbyScene_To_Player: You've joined game room: ${gameId}`,
    });

    // Notify only players within the same room that a new player has joined
    socket.to(gameId).emit("playerJoinedRoom", {
      message: `lobbyScene_To_GameRoom: Player with ID ${socket.id} has successfully joined your game '${gameId}'! `,
    });
  });

  socket.on("playerRespawned", (data) => {
    socket.to(data.gameId).emit("playerRespawn", data);
  });
